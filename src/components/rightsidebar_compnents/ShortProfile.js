...
const ShortProfile = () => {
  ...
  return (
    ...
    <div className="flex flex-col justify-center">
      {isFollowing ? (
        <button onClick={toggleFollow} className=" border-[1px] py-2 border-white px-7 w-full text-sm text-white  rounded-full font-semibold">
          Unfollow
        </button>
      ) : (
        <button onClick={toggleFollow} className="bg-[#7501E9] py-2 px-7 w-full text-sm text-white border-none rounded-full font-semibold">
        Follow
      </button>
      )}
